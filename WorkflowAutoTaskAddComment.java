public class WorkflowAutoTaskAddComment extends AutomationTest {
    ...
    public void executeAddCommentAutoTask() {
        prepareData();
        Workflow.Rest.startProcess(workflow);
        navigatelToEntity(workflow);
    ...
    @Test
    public void updateReleaseProcessViaAddCommentAA() {
        setAPIUserToWorkflow();
        navigatelToEntity(stage);
        reporter.testStep("Add Auto Task - Add Comment with Entity type = \"This Workflow\"");
        String thisWorkflow = "This Workflow";
        String name = "addComment_ThisReleaseProcess_" + RandomStringUtils.randomAlphabetic(5);
        UI.Views.workflowStageView.stageItemsContainer.toolbar.openToolbarAddAutoTaskDialog();
    ...
    private void setAPIUserToWorkflow() {
        workflow = Workflow.Rest.update(workflow, Workflow.Fields.API_KEY, Workflow.Rest.createAPIUserForProcess(RoleName.WORKSPACE_ADMIN.getValue()));
    }

    private String getCompleteMessage(int numberItems) {
        return "Auto action has successfully added a comment to " + numberItems + " items.";
    }

    private void retryAutoTaskWithAPIUserAndCheck(ReleaseProcessBase.Data autoTask, int numberItems) {
